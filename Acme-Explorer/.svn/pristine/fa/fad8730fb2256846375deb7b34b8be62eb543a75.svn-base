...
public class TripServiceTest extends AbstractTest {
	...
	@Test
	public void testAssignedCategoryToTrip() {
		...
		category = this.categoryService.findOne(6292);

		tripSaved = this.tripService.assignedCategoryToTrip(trip, category);

		Assert.isTrue(tripSaved.getCategory().equals(category));
		Assert.isTrue(category.getTrips().contains(tripSaved));

		super.unauthenticate();
	}

	@Test
	public void testUnassignedStageOfTrip() {
